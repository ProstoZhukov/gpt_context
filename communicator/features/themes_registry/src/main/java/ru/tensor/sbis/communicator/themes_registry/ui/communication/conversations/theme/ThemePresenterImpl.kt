...
import ru.tensor.sbis.communicator.common.conversation_preview.ConversationPreviewMenuAction.ThemeConversationPreviewMenuAction.DeleteGroup
import ru.tensor.sbis.communicator.common.conversation_preview.ConversationPreviewMenuAction.ThemeConversationPreviewMenuAction.GoToConversation
...
import ru.tensor.sbis.communicator.common.conversation_preview.ConversationPreviewMenuAction.ThemeConversationPreviewMenuAction.Report
...
import ru.tensor.sbis.communicator.common.conversation_preview.ConversationPreviewMenuAction.ThemeConversationPreviewMenuAction.Unpin
...
internal class ThemePresenterImpl(
    ...
    init {
        ...
        if (isSharingMode) {
            listFilter.limitItems = MAX_ITEMS_FOR_SHARING
        } else {
            collectTabSelection()
        }
    ...
    override fun isImportContactsAllowed(): Boolean = !isSharingMode && importContactsHelper != null &&
    ...
    private fun updateListItemsLayoutRules() {
        ...
        if (isChannelTab()) {
            mView?.updateListItemsLayoutRules(
                showHeaderDate = true,
    ...
    override fun viewIsResumed() {
        viewIsResumed = true
        super.viewIsResumed()
        if (isSharingMode) mView?.setFabVisible(!checkHelper.isCheckModeEnabled)
        showBottomNavigation()
        hideNewMessagePushes()
        if (!isChannelTab()) {
            checkAndScheduleMarkAllRegistryItemsAsViewed()
    ...
    override fun onBranchTypeTabClick(navxId: NavxIdDecl) {
        ...
        mView?.run {
            ...
            if (customizationOptions.splittingChannelsAndDialogsEnabled && !isSharingMode) {
    ...
    override fun onSearchQueryChanged(searchQuery: String) {
        ...
        if (newSearchQuery.isNotBlank() && !isSharingMode) {
    ...
    private fun initSubscriptions(
        ...
        unreadCounterProvider.counterEventObservable
            ...
            .subscribe(
                {
                    if (!isSharingMode) mView!!.setUnreadAndUnviewedCounters(
                        it.unreadChats,
                        it.unreadDialogs,
                        it.unviewedChats,
                        it.unviewedDialogs
            ...
            hasSelection             -> {
                ...
                router.openContentScreen(
                    ThemeConversationParams(
                        ...
                        isArchivedDialog =
                            if (conversation.isChatForOperations) ChatType.HIDDEN == chatType
                            else DialogType.DELETED == dialogType
                    )
                )
            }
    ...
    override fun onScrollToTopPressed() {
        if (mDataListOffset != 0) makeSearchRequest()
        else resetUI()
    }
