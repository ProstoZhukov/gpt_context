...
import ru.tensor.sbis.crud3.R as RCrud
import ru.tensor.sbis.crud3.test.R as RTest
...
class ListComponentViewLayoutTest {
    ...
    fun `When layout has child in ListComponentView, then component has correct layout`() {
        createScenario().onActivity {
            ...
            assert(
                listComponentView.getChildAt(0).id == RCrud.id.crud3_refresh_layout_id ||
                    listComponentView.getChildAt(0).id == RCrud.id.crud3_list_container_id
            )
        }
    }
    ...
    fun `When layout has child in ListComponentView, then child move to inner container`() {
        createScenario().onActivity {
            val listContainer = it.findViewById<FrameLayout>(RCrud.id.crud3_list_container_id)
...
class TestListComponentViewLayoutActivity : AppCompatActivity(RTest.layout.list_component_view_activity) {
