...
import ru.tensor.sbis.version_checker.data.VersioningSettingsHolder.Companion.APP_DEBUG_SUFFIX
...
internal class InstallerManager @Inject constructor(
    ...
    override fun openMarket(targetPackageName: String, context: Context) {
        val targetAppId = targetPackageName.replace(APP_DEBUG_SUFFIX, "")
        updateFactory.create(targetAppId).run(context)?.also {
