...
internal class VersioningLocalCache @Inject constructor(
    ...
    fun isRecommendationExpired(): Boolean = preferences.run {
        when {
            getBoolean(IS_UPDATE_ON_NEXT_SESSION_KEY, false) ->
                SESSION_ID.toString() != preferences.getString(LAST_SESSION_ID_KEY, "")

            contains(NEXT_TIME_FOR_RECOMMENDATION_KEY) && settingsHolder.checkRecommendationInterval ->
                getCurrentTime {
    ...
    fun postponeUpdateRecommendation(isPostponedByButton: Boolean) {
        preferences.edit().run {
            val isPostponed = isPostponedByButton && settingsHolder.checkRecommendationInterval
            if (isPostponed) {
                ...
                remove(IS_UPDATE_ON_NEXT_SESSION_KEY)
            } else {
                putBoolean(IS_UPDATE_ON_NEXT_SESSION_KEY, true)
