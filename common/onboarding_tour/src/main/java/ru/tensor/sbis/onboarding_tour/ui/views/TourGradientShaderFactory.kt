 ...
 */
fun createTourGradientDrawable(context: Context): Drawable = PaintDrawable().apply {
    shape = RectShape()
    val brandColor = OtherColor.BRAND.getValue(context)
    shaderFactory = TourGradientShaderFactory(brandColor)
}

/**
 * Фабрика создания градиента с несколькими переходами в разных точках экрана и разной прозрачностью цвета [color].
 */
private class TourGradientShaderFactory(@ColorInt var color: Int) : ShapeDrawable.ShaderFactory() {
