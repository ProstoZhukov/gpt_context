...
import ru.tensor.sbis.our_organisations.presentation.view.OurOrgNecessaryChoiceSelectionWindow
 ...
 * @author mv.ilin
 */
internal class OurOrgWithNecessaryChoiceSelectionWindowContract(
    private val ourOrgItemType: OurOrgItemType,
    override val requestKey: String = DEFAULT_REQUEST_KEY
) : DefaultFragmentContract<OurOrgSelectionWindowFactory, OurOrgNecessaryFragmentResult>() {

    override fun extractResult(data: Bundle) = OurOrgNecessaryChoiceSelectionWindow.extractResult(data)

    override fun getFactory() = OurOrgSelectionWindowFactory { params ->

        val creator = OurOrgNecessaryChoiceSelectionWindow.Creator(
