...
import ru.tensor.sbis.common.util.urldetector.detection.CharUtils;
...
public class HostNormalizer {
    ...
    private String mNormalizedHost;
    ...
    private void normalizeHost() {
        ...
        mBytes = tryDecodeHostToIp(host);

        if (mBytes != null) {
            InetAddress address;
            try {
                address = InetAddress.getByAddress(mBytes);
        ...
        mNormalizedHost = UrlUtil.encode(host).replace("\\x", "%");
    }
     ...
     * <p>
     * if ipv4 was found, mBytes is set to the byte representation of the ipv4 address
     */
    private byte[] tryDecodeHostToIPv4(String host) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            if (numParts == 4) {
                bytes[IPV4_MAPPED_IPV6_START_OFFSET + i] = (byte) section;
            } else { //numParts == 1
     ...
     * <p>
     * if ipv6 was found, mBytes is set to the byte representation of the ipv6 address
     */
    private byte[] tryDecodeHostToIPv6(String host) {
        String ip = host.substring(1, host.length() - 1);
        List<String> parts = new ArrayList<>(Arrays.asList(ip.split(":", -1)));
        if (parts.size() < 3) {
            return null;
        }
