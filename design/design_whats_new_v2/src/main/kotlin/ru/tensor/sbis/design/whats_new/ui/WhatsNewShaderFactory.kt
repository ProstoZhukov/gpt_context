 ...
 */
internal fun createWhatsNewGradientDrawable(context: Context): Drawable = PaintDrawable().apply {
    shape = RectShape()
    val brandColor = OtherColor.BRAND.getValue(context)
    shaderFactory = WhatsNewShaderFactory(brandColor)
}

/**
 * Фабрика создания градиента с несколькими переходами в разных точках экрана и разной прозрачностью цвета [color].
 */
private class WhatsNewShaderFactory(@ColorInt var color: Int) : ShapeDrawable.ShaderFactory() {
