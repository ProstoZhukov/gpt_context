...
package ru.tensor.sbis.design.utils.viewdraghelper;
...
/**
 * Копия класса из androidx.customview.widget, модифицированная для возможности переопределения максимальной
 * длительности анимации перемещения view
 *
 * ViewDragHelper is a utility class for writing custom ViewGroups. It offers a number
 * of useful operations and state tracking for allowing a user to drag and reposition
 * views within their parent ViewGroup.
 */
@SuppressWarnings({"PointlessBitwiseExpression", "unused", "Convert2Lambda"})
public class ViewDragHelper {
    ...
    private static final int DEFAULT_MAX_SETTLE_DURATION = 600; // ms
    ...
    private int mMaxSettleDuration = DEFAULT_MAX_SETTLE_DURATION;
    ...
    /**
     * Задаёт максимальное время анимации перемещения view до целевой позиции.
     *
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        return Math.min(duration, mMaxSettleDuration);
    }
    ...
    private boolean isValidPointerForActionMove(int pointerId) {
        if (!isPointerDown(pointerId)) {
            Timber.e("Ignoring pointerId=" + pointerId + " because ACTION_DOWN was not received "
    ...
    /**
     * TODO 15.06.21: https://online.sbis.ru/opendoc.html?guid=ad13024e-707b-4ce8-9688-efd6d0f573e5
     */
    private void runSafeWithAnalytics(@NonNull Runnable action, @NonNull MotionEvent event) {
      try {
          action.run();
      } catch (RuntimeException e) {
          Timber.e(e, "action index: %d, action pointerId: %d, mActivePointerId: %d, active pointer index: %d, mDragState %d",
                  event.getActionIndex(), event.getPointerId(event.getActionIndex()),
                  mActivePointerId, event.findPointerIndex(mActivePointerId), mDragState);
      }
    }
}