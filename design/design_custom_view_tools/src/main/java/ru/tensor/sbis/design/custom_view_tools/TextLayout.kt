...
import ru.tensor.sbis.design.custom_view_tools.utils.*
...
import ru.tensor.sbis.design.custom_view_tools.utils.layout.LayoutFactory.Companion.RTL_SYMBOLS_CHECK_COUNT_LIMIT
...
class TextLayout private constructor(
    ...
    /**
     * @param paint [TextPaint] для отрисовки разметки.
     * Использовать исключительно для оптимизаций в сценариях, где Вы используете собственный [TextPaint],
     * и стандартный [TextLayoutParams.paint] создается зря.
     * @param config настройка параметров текстовой разметки.
     * @see TextLayoutParams
     */
    constructor(paint: TextPaint, config: TextLayoutConfig? = null) : this(TextLayoutParams(paint = paint), config)
            ...
            get() = _width
                ?: when {
                    ...
                    params.layoutWidth != null -> params.layoutWidth!!
                        ...
                        val layoutWidth =
                            if (params.needHighWidthAccuracy) {
                                var maxLineWidth = 0f
                                repeat(requireLayout().lineCount) { lineIndex ->
                                    maxLineWidth = requireLayout().getLineWidth(lineIndex).coerceAtLeast(maxLineWidth)
                                }
                                maxLineWidth.roundToInt()
                            } else {
                               requireLayout().width
                            }
