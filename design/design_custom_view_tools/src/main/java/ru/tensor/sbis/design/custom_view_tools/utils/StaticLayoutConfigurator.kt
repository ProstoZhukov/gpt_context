...
import ru.tensor.sbis.design.custom_view_tools.utils.StaticLayoutConfigurator.Companion.createStaticLayout
...
class StaticLayoutConfigurator internal constructor(
        ...
        return buildStaticLayout().let { layout ->
            if (layout.lineCount <= maxLines) {
                ...
                text = SpannableStringBuilder(text.subSequence(0, endIndex))
                    .apply { if (!text.hasSymbolEllipsize && ellipsize == TruncateAt.END) append(ELLIPSIS_CHAR) }
    ...
    private fun configureText() {
        ...
        text = when {
            maxLines == MAX_LINES_NO_LIMIT -> text.highlightText(highlights)
            ...
            else -> text.ellipsizeAndHighlightText(paint, width * maxLines, highlights)
        }
    }
