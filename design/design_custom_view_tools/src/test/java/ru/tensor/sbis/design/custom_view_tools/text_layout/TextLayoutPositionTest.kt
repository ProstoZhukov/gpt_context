...
class TextLayoutPositionTest {
    ...
    fun `When call layout() and text is empty, then all positions is equals positions from layout()`() {
        ...
        textLayout.layout(left, top)
        val bottom = top + textLayout.height

        assertEquals(left, textLayout.left)
        assertEquals(left, textLayout.right)
        assertEquals(top, textLayout.top)
        assertEquals(bottom, textLayout.bottom)
    ...
    fun `Text position is equals 0 before first call layout()`() {
        ...
        val textPos = textLayout.stateSnapshot.textPos
    ...
    @Test
    fun `TextLayout baseline has Layout baseline`() {
        val layoutBaseline = textLayout.baseline
        val staticBaseline = textLayout.stateSnapshot.layout!!.getLineBaseline(0)
    ...
    @Test
    fun `When TextLayout isVisible is equals false, then baseline is equals Layout baseline`() {
        val layoutBaseline = textLayout.baseline
        val staticBaseline = textLayout.stateSnapshot.layout!!.getLineBaseline(0)
